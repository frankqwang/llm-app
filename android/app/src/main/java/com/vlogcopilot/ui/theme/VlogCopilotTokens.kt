 ...
 *
 * Design tokens for VlogCopilot — colors, shapes, spacing, motion. Borrows
 ...
 * vlogcopilot package so the rest of the gallery app keeps its existing
 * Material brand untouched.
 *
 * Usage:
 *   VlogCopilotTheme {
 *     // your composables can read MaterialTheme.colorScheme as usual,
 *     // or VlogCopilotTokens.spacing.lg etc. for the extended tokens
 *   }
 */
...
internal val LocalVlogCopilotSpacing = staticCompositionLocalOf { VlogCopilotSpacing() }
...
internal val LocalVlogCopilotMotion = staticCompositionLocalOf { VlogCopilotMotion() }

// ---------------- Extended palette (for tinted buttons / chips / separators) ----------------

@Immutable
data class VlogCopilotExtraColors(
...
internal val LocalVlogCopilotExtraColors = staticCompositionLocalOf {
...
object VlogCopilotTokens {
  val spacing: VlogCopilotSpacing
    @Composable @ReadOnlyComposable get() = LocalVlogCopilotSpacing.current

  val motion: VlogCopilotMotion
    @Composable @ReadOnlyComposable get() = LocalVlogCopilotMotion.current

  val colors: VlogCopilotExtraColors
    @Composable @ReadOnlyComposable get() = LocalVlogCopilotExtraColors.current

  val shapes get() = VlogCopilotShapes
}
