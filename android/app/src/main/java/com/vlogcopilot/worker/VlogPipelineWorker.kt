...
class VlogPipelineWorker(
  ...
  override suspend fun doWork(): androidx.work.ListenableWorker.Result {
    ...
    val runConfig = VlogCopilotRunConfig.load(applicationContext)
    ...
    return try {
      val gemma = VlogCopilotModelRegistry.resolve(applicationContext) ?: run {
        val lastName = VlogCopilotModelRegistry.lastEnqueuedModelName(applicationContext)
  ...
  private fun initialForegroundInfo(): ForegroundInfo {
    ...
    return buildForeground(buildNotif("VlogCopilot starting"))
  }
  ...
  companion object {
    const val WORK_NAME = "vlog_copilot_pipeline"
    ...
    const val ITERATION_WORK_PREFIX = "vlog_copilot_iterate_"
    ...
    const val CURATION_WORK_PREFIX = "vlog_copilot_curate_"
    ...
    const val KEY_ITERATION_EVENT_ID = "vlog_copilot.iteration_event_id"
    ...
    const val KEY_CURATION_REQUEST_ID = "vlog_copilot.curation_request_id"
    private const val NOTIF_ID = 24601
    private const val CHANNEL_ID = "vlog_copilot_progress"
    ...
    fun ensureChannel(context: Context) {
      ...
      mgr.createNotificationChannel(
        NotificationChannel(CHANNEL_ID, "VlogCopilot pipeline", NotificationManager.IMPORTANCE_LOW)
