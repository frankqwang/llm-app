...
import com.vlogcopilot.CharacterPlanner
import com.vlogcopilot.ProjectStore
import com.vlogcopilot.RhythmPlanner
import com.vlogcopilot.TemplateCatalog
import com.vlogcopilot.agents.AgentRuntime
import com.vlogcopilot.agents.AudienceAgent
import com.vlogcopilot.agents.CriticAgent
import com.vlogcopilot.agents.DirectorAgent
import com.vlogcopilot.agents.EditorAgent
import com.vlogcopilot.agents.IntentParserAgent
import com.vlogcopilot.agents.MontageAgent
...
import com.vlogcopilot.pipeline.UserCurationPlanner
import com.vlogcopilot.pipeline.VideoFrameSheetBuilder
import com.vlogcopilot.render.VideoRenderer
...
import com.vlogcopilot.schemas.IterationSummary
...
import com.vlogcopilot.schemas.UserBrief
...
class PipelineOrchestrator(
  ...
  suspend fun run(
    ...
    runConfig: VlogCopilotRunConfig = VlogCopilotRunConfig.load(context),
...
private suspend fun buildTimeline(
  ...
  private fun applyPatches(base: ShotRequest, patches: Map<String, String>): ShotRequest {
    ...
    for ((k, v) in patches) {
      when (k.trim().lowercase()) {
        "role" -> if (v.isNotBlank()) runCatching { role = com.vlogcopilot.schemas.ShotRole.valueOf(v.uppercase()) }
  ...
  private suspend fun runEvent(
    ...
    try {
      ...
      val critiques = mutableListOf<com.vlogcopilot.schemas.Critique>()
      var working = v1
      var lastCritique: com.vlogcopilot.schemas.Critique? = null
      var aborted = false
      ...
      Log.d(TAG, "perf ${event.eventId}: encoder=${com.vlogcopilot.render.EncoderProbe.selectedEncoderName()} total=${perf.totalMs}ms browse=${perf.browseMs} audience=${perf.audienceMs} director=${perf.directorMs} editor=${perf.editorMs} critic=${perf.criticMs} render=${perf.renderMs}")
      if (out != null) {
        DecisionStore.writeTimelineFinal(context, event.eventId, out.timeline)
        outputs[event.eventId] = out.outputPath
        IterationStore.recordInitialVersion(context, event.eventId, out.outputPath)
