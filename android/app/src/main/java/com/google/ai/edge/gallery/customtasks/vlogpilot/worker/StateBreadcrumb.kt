...
object StateBreadcrumb {
  ...
  fun fromProgress(context: Context, p: PipelineProgress) {
    when (p) {
      ...
      is PipelineProgress.Perceiving -> mark(context, "perceive", "${p.current}/${p.total}")
      is PipelineProgress.Annotating -> mark(context, "annotate", "${p.current}/${p.total}")
      is PipelineProgress.EventStart -> mark(context, "event_start", "${p.eventId} ${p.index}/${p.total}")
      is PipelineProgress.EventStage -> mark(context, "event_stage", "${p.eventId} ${p.stage}")
